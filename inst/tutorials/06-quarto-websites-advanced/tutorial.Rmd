...
<!-- Should we explain what usethis::use_github_pages() pages id doing, perhaps spreading out the information various knowledge drops? -->

<!-- Consider adding more information in knowledge drops about alternate ways to make a website from the IDE, making new directory with git box checked, and then using `usethis::use_github()` to connect to Github. It is useful to see this work, even if it is not our default strategy.  -->
...
Depending on your computer setup, this command will require you to answer some questions. Do so as best you can. When asked what to open the website with choose "rstudio".
...
In the Source pane, open the `_quarto.yml` file. Change the title to `"Website 1"`. Save the file. The indentation before `title` should match that of `navbar` directly below it.
...
Open the `index.qmd` file in the Source pane. Replace the title with `"Homepage"`. Save the file.
...
<!-- DK: Fix this problem someday. -->

Note that we purposely misspelled `setup` as `setupx` because of a bug in the **learnr** package. You should, of course, spell it correctly.
