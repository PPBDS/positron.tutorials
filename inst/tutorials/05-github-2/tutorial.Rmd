...
author: David Kane
tutorial:
  id: positron-and-github-advanced
output:
  learnr::tutorial:
    progressive: yes
    'allow_skip:': yes
...
<!-- DK: Make a decision about the best way to deal with the restart of Positron which seems to  -->

<!-- DK: There should be four projects, each moving more quickly during the creation phase. The core coding is done with AI. -->

<!-- project-3 should require users to fork a repo. -->

<!-- In project 4, use the GitHub created .gitignore and discuss stuff there. -->
...
This tutorial covers more details in the usage of [Git](https://git-scm.com/) and [GitHub](https://github.com/). Some material is from [*R for Data Science (2e)*](https://r4ds.hadley.nz/) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund.
...
<!-- DK: Do we need these instructions given that we make a new window each time? That is, we never actually use the new Console in the window which is running the tutorial. Instead, give instructions about using the two windows approach each time. Mention that we are creating four projects. -->

Start a new Console by clicking the "Start a New Session" button --- the `+` sign --- on the Console tab. You will always need to do this when working on a tutorial because the default Console will be busy running the tutorial itself. Use the new Console for the relevant questions below. 


## Project 1
### 

<!-- DK: Project 1 should be slightly more sophisticated and project 2 should be slightly more. Project 1 should not only use code chunk options. It should not use execute or suppressPackage. Project 2 should be the reverse. Project 3 should do what Project 2 does, but without explicit instructions. Project 4 should fork. -->
...
We don't need to set up Git and Github again, having already done so. In this project, our goal is to publish a nice graphic of data about penguins.
...
Connect the `project-1` Github repo to a project on your computer. Recall the steps:
...
Recall that you can add a code chunk by using the shortcut key combination: `Command + Option + i` (Mac) or `Ctrl + Alt + i` (Windows).
...
Ask your favorite AI to create a beautiful graphic using the `penguins` data. Phrase the question however you like. I [asked](https://chatgpt.com/c/67d4e9fb-48cc-8002-abe5-dc29fba775e7) [chattGPT](https://chatgpt.com/):
...
<!-- DK: Could add some screen shots from GitHub Pages site to help understand what is going on. -->
...
<!-- DK: Not sure this explanation is correct. -->

## Project 2
### 

<!-- DK: Project 2 should be a bit faster. For example, exercises 1 and 2 should be combined. -->
...
Positron will automatically restart and place you within the `project-2` R project. 
...
<!-- DK: Huh? This does not seem to connect. After all, isn't quarto-2.qmd already empty beneath the YAML header? Also, this is not the code we created in the previous section. Instead, we should be using AI and a new data set. -->
...
`quarto-2.html` is unchanged. Label'ing a code chunk does not affect the output, but it is still good practice.
...
<!-- DK: Switch to quarto publish gh-pages quarto-2.qmd. -->
...
Check out the Git tab. Note the "M" next to `quarto-2.qmd` and `quarto-2.html`. The "M" is Git's (and Positron's) way of telling us that the files have been **M**odified. Note, also, the new directory, `rsconnect`. This is another directory, like `quarto-2_files`, which does not belong on GitHub.
