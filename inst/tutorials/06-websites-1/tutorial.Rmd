...
`quarto publish`, like `quarto preview`, first renders all the files on the project. Note the phrase "Rendering for publish" at the bottom of the output. The URL for the website is provided at the end.
...
The `.quarto` directory is like the `.git` directory. We will never look at or change the files inside of them. Only the relevant programs --- Quarto and Git --- should work with the contents of these directories. 
