...
<!-- PG: replace "code echos" with "code echoes" -->
...
<!-- PG: add mode = "wb" to ensure that the sheet is seen as a binary file when installed b/c R assumes it is a text file by default. This will prevent corruption of the file, requiring the student to go back and forth with AI multiple times to get the solution.-->
In the Console, run:
...
### Exercise 9
<!-- PG: remove "add" between "and" and "use" -->
...
<!-- PG: Replace AI prompt with "Create a scatter plot from wine_tibble with Phenols vs Flavanoids, colored by Color. Include jittered points, a LOESS smooth curve, and label the maximum Flavanoids value as 'Outlier' in magenta. Use blue-to-orange color gradient and classic theme." -->
