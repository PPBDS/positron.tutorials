...
<!-- Posit Cloud works much less well. No reason to switch to this anytime soon.  I could not get quartopub.com to work at all. -->

## Introduction
### 

This tutorial covers how to create [Quarto documents](https://docs.posit.co/ide/user/ide/guide/documents/quarto-project.html#creating-new-documents) and how to publish your work to the web using [Rpubs](https://rpubs.com/). Some material is from [*R for Data Science (2e)*](https://r4ds.hadley.nz/) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund.
...
Be careful of the formatting in the YAML header. It is very fussy! For example, if you forget to indent the `echo: false` line, you will get an error.
...
<!-- DK: In the olden days, it was possible to replace an Rpubs document on-the-file. That does not seem to work anymore. Once you upload something with slug X, then you can't change that html. You need to delete the page if you want to use slug X again. Since we don't use Rpubs once we learn Quarto websites, I am not going to worry about that. -->
...
There are other free (and paid) services for publishing on the web. In the website tutorials in this package, we use [GitHub Pages](https://pages.github.com/).


### Exercise 2

Render the `quarto-4.qmd` file to ensure that it works. (If you never got a diamonds plot working, just go back to simply printing out the `diamonds` tibble)
...
Along with other files, you should see a `quarto-4.html` file, which was generated after you rendered your QMD. (Note that the "qmd" file extension stands for **Q**uarto **M**arkdown **D**ocument.)

### Exercise 3

The page is probably not as nice as it could be because the code for your plot is displayed. That is ugly. No one wants to see your code. Also, "My Fourth Quarto Document" is a somewhat stupid title.

Change the YAML header to be:
...
Also, delete the `#| echo: false` from the first code chunk since it is no longer necessary. 

In the Console, run:

````
tutorial.helpers::show_file("quarto-4.qmd", end = 6)
````
...
`Cmd/Ctrl + Shift + K` to render the document to confirm the changes. It is good to make your page look as good as possible before putting it up on the web
...
````
quarto::quarto_publish_doc(input = "quarto-4.qmd", server = "rpubs.com")
````

This will render the document, specified with the `input` argument, locally in order to create an HTML which will then sent to the `server`, which is RPubs.


You will be taken to the RPubs website. (You may need to authorize Positron to open the Rpubs link. You may need to sign into your account.) 

* By default, the "Title" of the document on Rpubs should be the same as the `title` in the YAML header. You can change it if you like. 

* You can leave the description blank if you don't want to write one.

* Add a meaningful slug (a unique website name that's often just the name of your project), otherwise you will end up with an ugly, long address you didn't choose and can't remember. We recommend `diamonds`.
...
This tutorial covered how to create [Quarto documents](https://docs.posit.co/ide/user/ide/guide/documents/quarto-project.html#creating-new-documents) and how to publish your work to the web using [Rpubs](https://rpubs.com/). Some material was from [*R for Data Science (2e)*](https://r4ds.hadley.nz/) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund.
