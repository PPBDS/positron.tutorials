...
Start by clicking on the Terminal tab in Positron. This is next to the Console tab, within the Console pane.
...
Your answer will be different, not least because your username is unlikely to be `dkane`. This is the location on your computer where the R program started up when you opened Positron.
...
What are the names of the [four default panes](https://docs.posit.co/ide/user/ide/guide/ui/ui-panes.html) in Positron?
...
-   Paste the link you copied from GitHub into `Repository URL`. Make sure your project is in the `projects` directory you made in the "Positron and Code" tutorial in order to maintain a single location in which you have placed all your R projects. If it is not, click on "Browse...", and select your `projects` folder.
...
This last step will cause Positron to restart. Go ahead and follow these steps now. Then, return to the tutorial.
...
In the "Positron and Code" tutorial, we saw that an R project begins with just one file, the `Rproj` file, which is `First_Repo.Rproj` in this case. But, because this R project started by connecting to a GitHub repo --- and because we initialized that repo with a README --- we also have a `README.md` file.
