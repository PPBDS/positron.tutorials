...
<!-- PG: Addressed this, but created new exercise so that the student can find this out themselves -->
...
<!-- PG: Shouldn't they run "show_file("analysis.qmd", chunk = "Last")" or type dbplyr_names after running `Cmd/Ctrl + Enter` -->
```         
 dbplyr_names <- tbl(con_names, "babynames")
```
...
<!-- PG: Why would we replace code in current cell, wouldn't it be in a new cell, otherwise the tibble for x would get removed. -->
In the Console, run:
...
### Exercise 5
<!-- PG: It would be dbplyr_fly not flights, right? Also, then "flights$tailnum" wouldn't work, so maybe just remove that chunk completely and/or just keep the ask AI, which will use dbplyr_fly? I had to use class(pull(dbplyr_fly, tailnum)) -->
...
<!-- PG: What are the string knowledge drops and the paragraphs after that for? -->
```         
