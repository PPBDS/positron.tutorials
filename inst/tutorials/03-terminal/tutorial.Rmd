...
<!-- Changing this around so that now there are two files, which live in images/, which the student will download and then use in the tutorial. The first, download.txt, just teaches them how to download a file. The second, example.txt, is just used for grepping practice. We are removing the use of $R_HOME because that variable does not exist outside the R session. -->
...
<!-- DK: Delete this question. -->

<!-- DK: Is the behavior for the above and this question different on Windows? -->

Run `ls -l $R_HOME/`. Just adding the trailing forward slash (or backward slash in Windows?) changes the output.  CP/CR.
