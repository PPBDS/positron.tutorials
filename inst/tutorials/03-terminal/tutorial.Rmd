...
<!-- Put a text file in images/ instead of using TODO.txt file. -->

<!-- I should better understand what the RTools environment looks like. Maybe I should just assume that they have done so (and check that they did) and then only worry about the minor differences between it and Mac/Linux.  -->
...
<!-- Make sure that everything makes sense to a student using Windows. Note all the DK comments which mention adding Windows explanations. Do more to make Windows users more comfortable. They shouldn't have to translate! -->

<!-- Several discussions about `..` are redundant. -->

<!-- Double discussion of / versus \. -->
...
<!-- Oh no! Looks like $R_HOME does not exist in the Terminal in Positron! I suspect that this is because R_HOME is only created within R. So Sys.getenv("R_HOME") works from the Console, but `echo $R_HOME` produces nothing from the Terminal. Need to do something about this! -->

<!-- R RHOME from the terminal does give us the R home directory. Cross-platorm, I think. Useful? I could also create the R_HOME environment variable by hand at the start of the tutorial. Or could create it in .RProfile. -->

<!-- Is working with the R home directory worthwhile? Is there some other file that exists on Mac and Windows?  Maybe we could use R.home() to determine the location and then require the student to create the entire path by hand? Or we could just download another file from the repo. Don't forget to include it in .Rbuildignore -->
...
<!-- DK: Could have them ask ChatGPT to do something and then do it. -->
...
Note that `rm -rf symbols` and `rm -rf symbols/`, with the trailing forward slash, have the same effect. Except in unusual circumstances, including the trailing slash in the name of a directory has no effect.
...
There are many different environmental variables. You can also make your own.
