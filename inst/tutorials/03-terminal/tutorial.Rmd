...
<!-- By default, RTools leaves Windows users using the Powershell. I have some notes in tutorial.helpers about changing this in "Tutorials in Positron." If we did this, then we could simplify a lot of the code below and stop with all the translation nonsense.  -->

<!-- Get rid of lots of the translation nonsense, like forward/backward slashes. -->
...
<!-- Make sure that everything makes sense to a student using Windows. Note all the comments which mention adding Windows explanations. Do more to make Windows users more comfortable. They shouldn't have to translate! -->
...
<!-- Always showing the correct answer afterwards is a good idea. But there are a score of questions in which we have failed to do this. Fix this. -->
...
Once you install RTools, you should restart your Positron session. If you are using Windows and are having trouble doing this, you may need to "translate" some if the instructions we give.
...
The tutorial defaults use Mac/Unix nomenclature. If you are using Windows, you may need to translate, for example:
...
The prompt acts as a quick way to tell you where in the computer you are. The Terminal is sensitive to which folder location you are in, just like the Console is.
...
The Actvity Bar (left side, colored blue) in Positron includes several buttons. When you click on of these buttons, a "pane" of some sort is opened next to the Activty Bar. (Pressing the button again closes the pain.)

The top one of which is the Explorer. Pressing it opens the Explorer pane, also called Project Explorer. This functions like the Mac Finder or the Windows Explorer. 
...
Note that `rm -rf symbols` and `rm -rf symbols/`, with the trailing slash, have the same effect. Except in unusual circumstances, including the trailing slash in the name of a directory has no effect.
...
Recall that, on Windows, we accomplish the same goal `echo %USERPROFILE%`. Windows does not usually have an `HOME` environmental value.
...
There are many different environmental variables. You can also make your own. My anwer looks like:
...
In the Terminal, run `ls -l $HOME`, so that we can see more information in a **l**ong list.  CP/CR.
...
Press the "Download raw file" button. This should place a copy of the file in your `Downloads` directory. At the Terminal, run `ls ~/Downloads/A*`. CP/CR.
...
`>` can be used to redirect the output of any command, not just `echo`. Let's try to make a new text file in `options` with just the first 28 lines of `AUTHORS.txt` called `AUTHORS-fake`. Run `head -n 25 AUTHORS.txt > AUTHORS-fake`. Run `ls` to confirm that it worked. CP/CR.
...
<!-- Too big a jump to this question? -->
...
<!-- Give a tour of what the different columns mean?  -->
...
The `*` is a wildcard character, meaning that the `ls` command should return all the files which begin with a capital `T`. For me, there are three such files in my `Downloads` directory. 
...
The trick with this command is the `>` pipes the results into a file, deleting anything already in he file, while `>>` appends to an already existing file. So, we are creating `files.txt` line-by-line.
...
````
kane@Davids-MacBook-Pro-3 grepping % echo hat.txt > files.txt & echo cat.txt >> files.txt & echo ate.txt >> files.txt & echo "" >> files.txt
[1] 98048
[2] 98049
[3] 98050
...
This should return three lines: 
