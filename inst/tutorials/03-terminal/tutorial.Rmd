...
<!-- Can we check for the installation of RTools from the Terminal? Might be nice to not have to use the Console at all in this tutorial? -->
...
If you are using Windows, you must install RTools. Download the latest version from [here](https://cran.r-project.org/bin/windows/Rtools/). Currently, the Rtools45 installer is what you want, assuming that your installed R version is at least 4.5.0. The link is in the seventh paragraph of [this page](https://cran.r-project.org/bin/windows/Rtools/rtools45/rtools.html). Use your favorite AI to help determine whether to use the Rtools45 installer or 64-bit ARM Rtools45 installer. 

Once you install RTools, you should restart your Positron session. If you are using Windows and are having trouble doing this, you may need to "translate" some of the instructions we give.

### Exercise 1

Start a second Positron window, as usual.

From the Console, run `pkgbuild::has_rtools()`. CP/CR.
...
This makes sense since we only RTools can only be installed on Windows machines. However, if you are on Windows, this should return `TRUE`. If it does not, install/fix RTools until you get `TRUE`, otherwise, much of this tutorial won't work for you.
