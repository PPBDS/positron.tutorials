...
<!-- DK: Oh no! Looks like $R_HOME does not exist in the Terminal in Positron! I suspect that this is because R_HOME is only created within R. So Sys.getenv("R_HOME") works from the Console, but `echo $R_HOME` produces nothing from the Terminal. Need to do something about this! -->

<!-- DK: R RHOME from the terminal does give us the R home directory. Cross-platorm, I think. Useful? I could also create the R_HOME environment variable by hand at the start of the tutorial. Or could create it in .RProfile. -->

<!-- Is working with the R home directory worthwhile? Is there some other file that exists on Mac and Windows?  Maybe we could use R.home() to determine the location and then require the student to create the entire path by hand? -->
...
The extra `.` at the end ensures that there is a blank line at the end of the file, which is always a good idea when dealing with text files. 
