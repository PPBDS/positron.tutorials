...
We generally work with Quarto directly at the Terminal. This is often referred to as *working from the command line*. Positron buttons, like "Preview," often call Terminal commands behind the scenes. 
...
You will see a nice plot, along with a lot of junk code and messages. Note that Positron might only redirect you to the plot the first time you render `index.qmd`. Going forward, you may need to affirmatively click on the correct tab on your browser or on the Viewer tab in the Secondary Activity Bar to see the newly rendered page.
...
<!-- DK: Is the really true in Positron? -->

You are generally free to label your chunks however you like, but there is one chunk name that imbues special behavior in Positron: `setup`. When you first open a Quarto document, the chunk named `setup`, if there is one, will be run automatically once, before any other code is run.
