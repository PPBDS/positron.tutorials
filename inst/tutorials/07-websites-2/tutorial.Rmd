...
<!-- DK: These is something wrong such that check_current_tutorial() does not work. -->
...
The `.quarto` directory does not yet exist because we haven't rendered any of our QMD files yet. But when we do, we don't any of the junk in that file to be backed up on GitHub.
...
Your answer might not match ours. That is OK! How the html display various by browser and screen dimension. The important thing is that, once we have rendered a website, a working version appears in the `_site` directory. 
...
Press the Source Control button and confirm that there are no changes. There shouldn't be since publishing does not (usually) change any of the files in the project.
...
Connect the `website-2` GitHub repo to a project on your computer. If needed, close the Positron Welcome window and start an R interpreter.
...
In the Termina, run `cat index.qmd`. CP/CR.
...
Save the file. `Cmd/Ctrl + Shift + K` to ensure that the plot looks good. If it looks bad, or there are some weird messages/warnings, use the AI to fix things.
...
To be honest, I am not totally sure what this plot is doing. And that is OK! I only iterated three times with Grok, asking it to fix a couple of errors and warnings. I could spend more time, both understanding the plot and changing it to be more interesting. The point is to get you to spend more time with AI.


### Exercise 8

Render the `index.qmd` with the "Render" button. It should show the plot. As always, you could render with `Cmd/Ctrl + Shift + K`. Shortcut keys are lit!
...
<!-- DK: Does below really happen? -->

This is an Positron feature. It knows that this directory is a website project and that, therefore, it is unnecessary to even mention the format, which is best controlled from `_quarto.yml`. 
...
Restart your R session with `Cmd/Ctrl + Shift + 0`. As always, this will require you to "Terminate Jobs" your tutorial.
...
From the Terminal, run `quarto publish gh-pages`. You will need to hit "Enter" at least once. CP/CR.
