...
<!-- DK: Personally, the first line was confusing to read. We might need to change this for users that aren't as fluent in English by breaking it down. We should replace it with "Choose an AI to use and in the chat, send a tibble of "billboard". " -->
...
<!--DK: Explain the warning message.  -->

###

For now, we will be working with functions from the **ggplot2** package.

When you're working with `ggplot()`, you typically won't use the `data` and `mapping` *arguments*. Instead, you will just rely on *position mapping* by which `ggplot()` assumes that the first item you pass in is the value for the `data` argument and the second item is the value for the `mapping` argument.
...
<!-- DK: Also, the sample code didn't have `mapping = aes(x = wk1)` exactly, so, we should add a line that addresses that and explains that in ggplot() command, first space is default set for data, then after comma, second space is set for mapping by default. -->
...
<!-- DK: You decide what to remove, what to move and what to add. -->
...
```{r billboard-100-6-test, echo = TRUE}
billboard |>
  filter(track %in% c("Dancing Queen", "Loser", "Try Again")) |>
  pivot_longer(
    cols = wk1:wk5,
...
```{r billboard-100-13-test, echo = TRUE}
billboard |>
  filter(wk1 < 101) |>
  arrange(wk1) |>
  mutate(improvement = wk1 - wk2, .after = wk2)
...
```{r billboard-100-14-test, echo = TRUE}
billboard |>
  filter(wk1 < 101) |>
  arrange(wk1) |>
  mutate(improvement = wk1 - wk2, .after = wk2) |>
...
```{r billboard-100-15-test, echo = TRUE}
billboard |>
  filter(wk1 < 101) |>
  arrange(wk1) |>
  mutate(improvement = wk1 - wk2, .after = wk2) |>
...
```{r billboard-100-16-test, echo = TRUE}
billboard |>
  filter(wk1 < 101) |>
  arrange(wk1) |>
  mutate(improvement = wk1 - wk2, .after = wk2) |>
...
<!--DK: Add part that says, if show_file gives error about function not existing, add library(tutorial.helpers) to first code chunk, the same one where library(tidyverse) is located. Also, give code that we used for the last code chunk. -->
...
```{r billboard-100-22-test, echo = TRUE}
ggplot(billboard_clean, aes(x = improvement)) +
  geom_histogram(binwidth = 5, fill = "skyblue", color = "black") + 
  labs(
	title = "Distribution of Billboard Top 100 Song Ranking Improvements From Week 1 to Week 2",
