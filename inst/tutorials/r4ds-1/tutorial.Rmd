...
### 
<!-- PG: Maybe add a line that after that says: One way to do that is by using GitHub -->

Professionals keep their data science work in the cloud because laptops fail.

### Exercise 2
<!-- PG: Maybe add a reminder like "Remember that a new code chunk can be created by using Cmd/Ctrl + Shift + I" -->
...
<!-- PG: We should add explanations to why we are using these commands. First, getwd() gives us our working directory, or the place the user is located. dir.create("data") creates the data folder within our working directory, or WD. list.files() displays the files inside whatever directory is inside the parenthesis. Since the area inside the parenthesis is blank, the command is executed for the current WD.
``` -->
...
###
<!-- PG: Add a line that says "You should get a tibble that looks like:" -->
...
<!-- PG: Are we going to explain the meanings of these variable category examples. If so, we should add those. Otherwise, we should say "We will discuss more about these in future tutorials" or give a link or something that the tutorial user can click on to learn more. So, something like "The LINK is a great way to learn more about this." -->

**Tabular data** is data organized in a table. A table is a group of cells, organized in rows and columns. 

### Exercise 2

<!-- PG: Personally, the first line was confusing to read. We might need to change this for users that aren't as fluent in English by breaking it down. We should replace it with "Choose an AI to use and in the chat, send a tibble of "billboard". " -->
In a prompt for AI provide a tibble of `billboard`. You only need the top 3 lines or so.
<!-- PG: I feel like giving the prompt to the tutorial user will remove the oppurtunity for the user to learn how to prompt AI, which is an essential skill. We should replace "Ask AI...and `geom_point()`." with "Using AI, create a simple graph plotting `wk1` against `wk2`. For the code, only use the functions `ggplot()`, `aes()`, and `geom_point()`." Additionally, since we are using `wk1` and `wk2`, we should explain what these variables mean & what we aim to do with the plot. -->
...
<!--PG: Should we add #|message : false to remove message. If not, we should explain why we didn't remove the warning message.  -->
###
...
<!-- PG: Also, the sample code didn't have `mapping = aes(x = wk1)` exactly, so, we should add a line that addresses that and explains that in ggplot() command, first space is default set for data, then after comma, second space is set for mapping by default. -->
...
<!-- PG: Capitalize the italicized LOESS -->
...
The `labs()` function takes in several arguments to modify the plot labels, including `x`, `y`, `title`, `subtitle`, and `caption`. The `x` and `y` arguments are used to modify the axis labels, while the `title`, `subtitle`, and `caption` arguments are used to modify the plot title, subtitle, and caption, respectively. 
...
<!-- PG: Since we are telling the user to replace `%>%` with `|>`, we should tell them to use "Ctrl/Cmd + H", which could be a useful shortcut for larger codechunks or projects. -->
...
<!-- We don't run this code because we don't like leaving a random png file lying around. We could delete it after creating it, but that also seems hacky. -->
...
`filter()` changes which rows are present without changing their order. `filter()` takes `billboard` as an input and produces a new tibble as its output.
...
<!-- PG: It should be you're, not your for the following sentence.-->
...
<!-- PG: It should be you're, not your for the following sentence. This is very repetitive and shouldn't be needed at all. -->
...
<!--PG: Add part that says, if show_file gives error about function not existing, add library(tutorial.helpers) to first code chunk, the same one where library(tidyverse) is located. Also, give code that we used for the last code chunk.-->
###
...
<!-- PG: Add line that reminds user to put blank line at end of .gitignore file. -->
...
###
<!-- PG: Add ", which are unexpected behaviors that lead to confusion" after common gotchas so the user understands what it "common gotchas" means. -->
...
###
<!-- PG: Replace "There exist many `theme_*()` functions" with "Many `theme_()` functions exist" -->
...
<!-- PG: Give our result for CP/CR, just for good measure. -->
###
...
<!-- PG: Say analysis.qmd instead of just QMD for clarity. Since it is a new section, we could also make a new qmd file, maybe name it "music.qmd".-->
...
<!-- PG: It should be you're, not your for the following sentence. This is very repetitive and shouldn't be needed at all. Also, add part that quickly explains what a character vector is.-->
...
<!-- PG: It should be you're, not your for the following sentence. This is very repetitive and shouldn't be needed at all. Also, all this feels like too much for the user to comprehend, even if the AI understands it. We can either break this down into more exercises or provide an explanation for what the point of the "Ask AI:" prompt was. Also, since there are two commands, "Ctrl/Cmd + Shift + Enter" needs to be done twice. If so, then update the echo on the bottom aswell.-->
...
###
<!-- PG: This direction should be part of a different exercise, most probably Exercise 9. -->
...
<!-- PG: Since it will below previous code, the cursor should be put on second function or just make user execute in console twice with the shortcut: Cmd/Ctrl + Shift + Enter. -->
...
<!-- PG: Since it will below previous code, the cursor should be put on the newly added function or just make user execute in console with the shortcut: Cmd/Ctrl + Shift + Enter until the newly added function is executed. -->
...
### Exercise 17
<!-- PG: Need to add step that tells user to add "values_drop_na = TRUE
  ) |>
  mutate(week = parse_number(week))" to bottom of code chunk after Exercise 16's code. -->
...
### Exercise 1
<!-- PG: Since we want clean versions of our plots, should we direct user to add "warning: false" to YAML header? -->
...
````
<!-- PG: Terminal may not be open for typing for the user because it is probably still running for render. So, we should provide user with ability to stop it by using "Ctrl/Cmd + C" -->
...
<!-- PG: Should we link the chapter of the book we were referring to for the user? -->
